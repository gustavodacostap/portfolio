...
export default function TaskManagerWeb() {
    const t = useTranslations("projects.holidayImporter");
    const p = useTranslations("projects");

    const currencyConverterImages = [
        "planilha-entrada.png",
        "log-execucao.png",
        "txt-resultado.png",
        "planilha-banco.png"
    ];
    ...
    return (
        <main className="pt-12 px-5 sm:px-6 md:px-12 lg:px-[10%] xl:px-[13%] 2xl:px-[15%]">
            <div className="relative w-full h-full aspect-video overflow-hidden rounded-2xl sm:rounded-4xl my-10">
                <Image
                    fill
                    alt="Holiday Importer preview"
                    src="/assets/images/projects/previews/holiday-importer.svg"
            ...
            <div className="flex flex-col md:flex-row md:gap-10">
                <div className="flex flex-col gap-5 mt-5 md:w-2/3">
                    <p>{t("description")}</p>
                    <Link href="https://github.com/gustavodacostap/ImportadorFeriados" target="_blank" className="text-right text-lg md:text-xl">
            ...
            <>
                {currencyConverterImages.map((img, index) => (
                    ...
                    >
                        <Image
                            alt={`Holiday Importer - ${img.replace(".png", "").replace(/-/g, " ")}`}
                            src={`/assets/images/projects/HolidayImporter/${img}`}
                            width={1911}
                            height={953}
                            quality={100}
            ...
            <footer className="my-5">
                <nav className="flex items-center justify-between">
                    <Link href="/projects/PasswordGenerator" className="font-medium text-lg md:text-xl">
                        {p("nextProject")}
                    </Link>
                    <Link href="/projects/PasswordGenerator" className="text-right font-medium text-lg md:text-xl">
                        {t("titleNextProject")}
                    </Link>
                </nav>
            </footer>
